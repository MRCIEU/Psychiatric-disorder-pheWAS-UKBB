ukbdatadir=Sys.getenv("UKBBDIR")
datadir=Sys.getenv("PROJECTDIR")
bridgedir=Sys.getenv("PROJECTDATADIR")
phenodatadir=paste0(datadir, "/data/confounder")
...
confs = read.table(paste(phenodatadir,'/confounderfile.csv', sep=""), sep=',', header=1)
colnames(confs) = c("eid","x31_0_0","x54_0_0","x54_1_0","x54_2_0","x21022_0_0","x22000_0_0")
print(dim(confs))
...
# id link file
bridge = read.table(paste0(bridgedir,"/data/data.7341.csv"), sep=',', header=1)
...
write.table(myconfs, paste0(phenodatadir,'/confounders-PHESANT-sensitivity-40pcs.csv'), sep=',', row.names=FALSE)
